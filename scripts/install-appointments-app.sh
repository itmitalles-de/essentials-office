#!/usr/bin/env bash
# Install the repository-owned Appointments app without touching application data.
set -Eeuo pipefail

SCRIPT_DIR="$(CDPATH='' cd -- "$(dirname -- "$0")" && pwd)"
PROJECT_DIR="$(CDPATH='' cd -- "$SCRIPT_DIR/.." && pwd)"
ENV_FILE=${NEXTCLOUD_ENV_FILE:-"$PROJECT_DIR/.env"}
SOURCE_DIR="$PROJECT_DIR/nextcloud-apps/appointments"
STAGE_DIR=
MAINTENANCE_ENABLED=false

die() {
  printf 'install-appointments-app: %s\n' "$*" >&2
  exit 1
}
...
cleanup() {
  ...
  if [ "$MAINTENANCE_ENABLED" = true ]; then
    if ! occ maintenance:mode --off >/dev/null; then
      printf 'install-appointments-app: WARNING: maintenance mode could not be disabled\n' >&2
      status=1
    fi
  fi
  if [ -n "$STAGE_DIR" ] && [[ "$STAGE_DIR" == */custom_apps/.appointments.stage.* ]] && [ -d "$STAGE_DIR" ]; then
...
if [ "${EUID}" -ne 0 ]; then
  exec sudo --preserve-env=NEXTCLOUD_ENV_FILE,APPOINTMENTS_BACKUP_DONE -- "$0" "$@"
fi
[ "$#" -eq 0 ] || die 'usage: install-appointments-app.sh'
for command in awk chown diff docker find jq mktemp mv rsync sudo tr; do
  command -v "$command" >/dev/null 2>&1 || die "required command not found: $command"
done
[ -f "$ENV_FILE" ] || die "missing environment file: $ENV_FILE"
[ -f "$SOURCE_DIR/appinfo/info.xml" ] || die 'repository-owned Appointments app source is incomplete'
...
target="$custom_apps/appointments"
...
[ ! -L "$target" ] || die 'refusing a symlinked Appointments app target'

STAGE_DIR=$(mktemp -d "$custom_apps/.appointments.stage.XXXXXX")
...
if [ -d "$target" ] && diff -qr --no-dereference "$STAGE_DIR" "$target" >/dev/null; then
  printf 'install-appointments-app: app code already matches\n'
  exit 0
fi

was_enabled=false
if occ app:list --output=json | jq -e '.enabled | has("appointments")' >/dev/null 2>&1; then
  was_enabled=true
  if [ "${APPOINTMENTS_BACKUP_DONE:-false}" != true ]; then
    "$SCRIPT_DIR/backup.sh"
  fi
  occ maintenance:mode --on >/dev/null
  MAINTENANCE_ENABLED=true
  occ app:disable appointments >/dev/null
fi
...
if [ "$was_enabled" = true ]; then
  occ app:enable appointments >/dev/null
  occ maintenance:mode --off >/dev/null
  MAINTENANCE_ENABLED=false
fi
printf 'install-appointments-app: versioned app code installed\n'
